...
Depending on available resources, instances can be created in two locations within the "ColoZüri" data center or the "NTT" data center in Rümlang. The location cannot be changed later. If you need to change the location, you will need to perform a database migration. See the [database migration guide](migrating-from-other-platforms#retrieving-and-restoring-databases) for more information.
